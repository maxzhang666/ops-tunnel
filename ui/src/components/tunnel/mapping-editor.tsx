        ...
        <Card key={idx}>
          <CardContent className="space-y-3 pt-4">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-muted-foreground">Mapping {idx + 1}</span>
              {value.length > 1 && (
            ...
            {(mode === 'local' || mode === 'remote') && (
              <div className="grid grid-cols-3 gap-3">
                <div className="col-span-2 space-y-1">
                  <Label className="text-xs">Connect Host</Label>
            ...
            {mode === 'dynamic' && (
              <>
                <div className="space-y-1">
                  <Label className="text-xs">SOCKS5 Auth</Label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <Label className="text-xs">Allow CIDRs (comma-separated)</Label>
                    <Input value={mapping.socks5?.allowCIDRs?.join(', ') ?? ''} onChange={(e) => onChange(updateMapping(value, idx, { socks5: { ...mapping.socks5!, allowCIDRs: e.target.value ? e.target.value.split(',').map((s) => s.trim()) : [] } }))} placeholder="10.0.0.0/8, 192.168.0.0/16" />
                  </div>
                  <div className="space-y-1">
                    <Label className="text-xs">Deny CIDRs (comma-separated)</Label>
            ...
            {(mode === 'local' || mode === 'remote') && (
              <div className="space-y-1">
                ...
                <Input value={mapping.notes ?? ''} onChange={(e) => onChange(updateMapping(value, idx, { notes: e.target.value }))} placeholder="e.g., PostgreSQL" />
              </div>
            )}
          </CardContent>
        </Card>
